...
import { ShieldCheck, Lock, Settings2, KeyRound, Crown } from 'lucide-react';
...
const App: React.FC = () => {
  ...
  // config.ts là nguồn sự thật duy nhất
  const contractData = MASTER_CONFIG;
  
  // Lưu trữ ảnh xác minh
  ...
  return (
    <div className="min-h-screen bg-[#f1f5f9] flex flex-col items-center py-10 px-4 font-main">
      {/* Admin Toggle */}
      <button 
        ...
        <div className="flex items-center gap-5">
          ...
          <div className="flex flex-col">
            <h1 className="text-2xl font-black text-slate-900 tracking-tighter uppercase leading-none">
              {contractData.headerBrandName} <span className="text-slate-400 font-bold">{contractData.headerBrandSuffix}</span>
            </h1>
            <span className="text-[10px] text-[#e11d48] font-bold uppercase tracking-[0.1em] mt-2">
              {contractData.headerSubtitle}
            </span>
          </div>
        </div>
        ...
        <div className="flex items-center gap-10">
          <div className="hidden lg:flex items-center gap-3 bg-white/50 px-5 py-2.5 rounded-full border border-white/50">
            ...
            <span className="text-[10px] font-black text-slate-400 uppercase tracking-widest">
              {contractData.headerStatusLabel}
            </span>
          </div>
          ...
          <div className="flex flex-col items-end gap-2">
            <div className="flex items-center gap-3">
              <span className="text-[10px] font-black text-slate-500 uppercase tracking-widest">
                {contractData.progressLabel}: {currentStepIndex}/5
              </span>
      ...
      <main className="w-full max-w-5xl bg-white shadow-[0_50px_100px_-20px_rgba(0,0,0,0.15)] rounded-[3.5rem] overflow-hidden border border-white relative transition-all duration-700">
        ...
        {step === AppStep.REVIEW && <ContractReviewView onContinue={() => setStep(AppStep.VERIFY)} data={contractData} isVerified={!!verificationImages.portrait} />}
        {step === AppStep.VERIFY && <VerificationView onComplete={(imgs) => { setVerificationImages(imgs); setStep(AppStep.SUCCESS); }} data={contractData} />}
        {step === AppStep.SUCCESS && <SuccessView data={contractData} portrait={verificationImages.portrait} />}
      </main>
      ...
      <footer className="mt-12 text-center no-print">
        <div className="flex items-center justify-center gap-10 opacity-30 grayscale hover:grayscale-0 hover:opacity-100 transition-all duration-500 mb-6">
          <div className="flex items-center gap-2">
            <ShieldCheck size={18} />
            <span className="text-[10px] font-black tracking-widest uppercase italic">{contractData.footerSecurityLabels[0]}</span>
          </div>
          <div className="flex items-center gap-2">
            <Lock size={18} />
            <span className="text-[10px] font-black tracking-widest uppercase italic">{contractData.footerSecurityLabels[1]}</span>
          </div>
        </div>
        <p className="text-[10px] text-slate-400 font-bold uppercase tracking-[0.4em]">
          {contractData.footerCopyright}
        </p>
      </footer>
      ...
      {showLogin && (
        <div className="fixed inset-0 z-[100] flex items-center justify-center bg-slate-900/40 backdrop-blur-md px-4">
          <div className="bg-white p-10 rounded-[2.5rem] shadow-2xl max-w-sm w-full text-center">
            ...
            <h3 className="text-xl font-black mb-6 uppercase tracking-tight">{contractData.adminLoginTitle}</h3>
